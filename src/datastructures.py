...
class Database:
    ...
    def _load_from_folder_helper(self, report, file_name):
        ...
        ip_regex = r'(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[1-9])\.(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])\.(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])\.(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])'
        for line in lines:
            columns = [i.replace(',', '') for i in line.split(' ') if i != '']
            if len(columns) == 0:
                continue
            elif re.fullmatch(ip_regex, columns[0]):
...
class Switch:
    ...
    '''
    This method reads the output generated by a show command and updates the port activity data for the switch object. The method creates a list of lines then loops 
    through the list starting at the first line that contains data about the switch ports. The way index_data_start identifies the first line of data was taken 
    ...
    def read_output(self, text: str, database: Database, epoch_days=time.time() // 86400):
        ...
        if re.match(start_of_data_regex, text):
            ...
            logging.error(f'Couldn\'t parse switch output for {self.switch_ip} which responded: {text}')
